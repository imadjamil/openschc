...
def create_echoreply(pkt, addr):
    ...
    ICMPv6Header = ICMPv6EchoReply(
        id=ECHO_REQUEST.id, seq=ECHO_REQUEST.seq, data=ECHO_REQUEST.data
    )

    Echoreply = IPv6Header / ICMPv6Header
    dprint("Echo reply at ping-device3.py: \n", Echoreply.show(dump=True))
            ...
            if ip_proto == 17:
                ...
                if udp_dport == socket_port:  # tunnel SCHC msg to be decompressed
                    ...
                    if rule[T_RULEID] == 6 and rule[T_RULEIDLENGTH] == 3:
                        ...
                        r = schc_machine.schc_recv(
                            device_id=device_id, schc_packet=schc_pkt
                        )
                        if (
                            r is not None
                        ):  # The SCHC machine has reassembled and decompressed the packet
                            dprint("ping_device.py, r =", r)
                            schc_pkt_decompressed = r[1]
                            pkt_reply, core_id = create_echoreply(
                                schc_pkt_decompressed, addr
                            )
                            uncomp_pkt = schc_machine.schc_send(
                                bytes(pkt_reply),
                                core_id=core_id,
                            )
                            dprint(uncomp_pkt)
