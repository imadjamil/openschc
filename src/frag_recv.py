...
import frag_msg
from frag_bitmap import (
    find_missing_tiles,
    sort_tile_list,
    find_missing_tiles_no_all_1,
    find_missing_tiles_mic_ko_yes_all_1,
...
class ReassembleBase:
    ...
    def get_mic(self, mic_target, extra_bits=0):
        ...
        dprint(
            "Recv MIC {}, base = {}, lenght = {}".format(
                mic.hex(), mic_target, len(mic_target)
            )
        )
        return mic
...
class ReassemblerNoAck(ReassembleBase):
    ...
    def receive_frag(self, bbuf, dtag):
        dprint(
            "state: {}, received fragment -> {}, rule-> {}".format(
        ...
        if schc_frag.fcn == frag_msg.get_fcn_all_1(self.rule):
            ...
            if schc_frag.mic != mic_calced:
                dtrace(
                    "ERROR: MIC mismatched. packet {} != result {}".format(
                ...
                dtrace(
                    "SUCCESS: MIC matched. packet {} == result {}".format(
    ...
    def get_state_info(self, **kw):
        result = {"type": "reassembly", "mode": "no-ack"}
        return result
...
class ReassemblerAckOnError(ReassembleBase):
    ...
    def receive_frag(self, bbuf, dtag):
        ...
        tiles = [
            schc_frag.payload.get_bits_as_buffer(tile_size) for _ in range(nb_tiles)
        ]
        dprint("---------tiles: ", tiles)
        ...
        if self.mic_received is not None:
            ...
            schc_packet, mic_calced = self.get_mic_from_tiles_received()
            dprint(
                "schc_frag.mic: {}, mic_calced: {}".format(schc_frag.mic, mic_calced)
            )
    ...
    def resend_ack(self, schc_frag):
        ...
        if self.mic_received is not None:
            schc_packet, mic_calced = self.get_mic_from_tiles_received()
            dprint(
                "schc_frag.mic: {}, mic_calced: {}".format(
        ...
        if self.state == "DONE":
            ...
            schc_ack = frag_msg.frag_receiver_tx_all1_ack(
                schc_frag.rule, schc_frag.dtag, schc_frag.win, cbit=1
            )
    ...
    def finish(self, schc_packet, schc_frag):
        ...
        # input('DONE')
        # decompression
        self.protocol.process_decompress(
            schc_packet, self.sender_L2addr, direction="UP"
        )
    ...
    def get_mic_from_tiles_received(self):
        ...
        if len(self.tile_list) > 1:
            ...
            if (
                self.tile_list[-1]["raw_tiles"].count_added_bits()
    ...
    def create_ack_schc_ko(self, schc_frag):
        ...
        if bit_list:
            ...
            dprint("missing wn={} bitmap={}".format(bit_list[0][0], bit_list[0][1]))
            # XXX compress bitmap if needed.
            # ACK failure message
            schc_ack = frag_msg.frag_receiver_tx_all1_ack(
                schc_frag.rule,
            ...
            # ACK failure message
            schc_ack = frag_msg.frag_receiver_tx_all1_ack(
                schc_frag.rule, schc_frag.dtag, win=0, cbit=0, bitmap=BitBuffer([])
            )
        else:
            ...
            dprint(
                "No missing tiles, sending last window: wn={} bitmap={}".format(
                    last_window, BitBuffer(window_list[last_window])
                )
            )
            # XXX compress bitmap if needed.
            # ACK failure message
