    ...
    def __reprx(self, t):
        if isinstance(t, str):
            ...
            return "{{{}}}".format(
                str(
                    ", ".join(
                        ['"{}": {}'.format(k, self.__reprx(v)) for k, v in t.items()]
                    )
                )
            )
        elif isinstance(t, list):
            return "[{}]".format(
                str(", ".join(["{}".format(self.__reprx(i)) for i in t]))
            )
        else:
            return repr(t)
...
class RuleManager:
        ...
        * the rule contains either one of a fragmentation and a compression description.
        ...
        for n_rule in sor:
            ...
            for e_rule in d["SoR"]:  # check no overlaps on RuleID
                left_aligned_e_ruleID = e_rule[T_RULEID] << (
                    32 - e_rule[T_RULEIDLENGTH]
                )
                if left_aligned_e_ruleID == left_aligned_n_ruleID:
                    dprint(
                        "Warning; Rule {}/{} exists not inserted".format(
                            bin(n_ruleID), n_ruleLength
                        )
                    )
                    overlap = True
                    break
    ...
    def _adapt_value(self, FID, value):
        if FIELD__DEFAULT_PROPERTY[FID]["TYPE"] == int:
            if type(value) != int:
                raise ValueError(
                    "{} TV type not appropriate for field {}".format(value, FID)
                )
            else:
                return value
        if FIELD__DEFAULT_PROPERTY[FID]["TYPE"] == str:
            if type(value) != str:
                ...
                return value
        if (
            FIELD__DEFAULT_PROPERTY[FID]["TYPE"] == bytes
        ):  # convert string with IPv6 address to bytes
            if type(value) is str:
                ...
                if addr.version != 6:  # expect an IPv6 address
                    raise ValueError(
                        "only IPv6 is supported, can not support {}".format(
                            addr.version
                        )
                    )
    ...
    def _create_fragmentation_rule(self, nrule):
        ...
        if not T_FRAG_DIRECTION in nrule[T_FRAG]:
            raise ValueError(
                "Keyword {} must be specified with {} or {}".format(
                    T_FRAG_DIRECTION, T_DIR_UP, T_DIR_DW
                )
            )

        if not nrule[T_FRAG][T_FRAG_DIRECTION] in [T_DIR_UP, T_DIR_DW]:
            raise ValueError(
                "Keyword {} must be {} or {}".format(
                    T_FRAG_DIRECTION, T_DIR_UP, T_DIR_DW
                )
            )
        ...
        if T_FRAG_MODE in nrule[T_FRAG]:
            ...
            if nrule[T_FRAG][T_FRAG_MODE] in [
                T_FRAG_NO_ACK,
                T_FRAG_ACK_ALWAYS,
                T_FRAG_ACK_ON_ERROR,
            ]:
                ...
                if nrule[T_FRAG][T_FRAG_MODE] == T_FRAG_NO_ACK:
                    ...
                    if nrule[T_FRAG][T_FRAG_PROF][T_FRAG_LAST_TILE_IN_ALL1] == True:
                        raise NotImplementedError(
                            "Last tile in All-1 is not implemented yet"
                        )
                ...
                _default_value(
                    ...
                    nrule,
                    T_FRAG_WINDOW_SIZE,
                    (0x01 << (arule[T_FRAG][T_FRAG_PROF][T_FRAG_FCN])) - 1,
                )
            else:
                raise ValueError(
                    "Unknown fragmentation mode", nrule[T_FRAG][T_FRAG_MODE]
                )
        else:
    ...
    def _create_compression_rule(self, nrule):
        ...
        for r in nrule[T_COMP]:
            ...
            if not r["FID"] in FIELD__DEFAULT_PROPERTY:
                raise ValueError(
                    "Unkwown field id {} in rule {}/{}".format(
            ...
            FID = r[T_FID]
            entry[T_FID] = FID
            entry[T_FL] = self._return_default(
                r, T_FL, FIELD__DEFAULT_PROPERTY[FID][T_FL]
            )
            entry[T_FP] = self._return_default(r, T_FP, 1)
            entry[T_DI] = self._return_default(r, T_DI, T_DIR_BI)
            if entry[T_DI] in [T_DIR_BI, T_DIR_UP]:
                up_rules += 1
            if entry[T_DI] in [T_DIR_BI, T_DIR_DW]:
                dw_rules += 1
    ...
    def Print(self):
        ...
        for dev in self._ctxt:
            ...
            for rule in dev["SoR"]:
                ...
                print(
                    "|Rule {:8}  {:10}|".format(
                        txt, self.printBin(rule[T_RULEID], rule[T_RULEIDLENGTH])
                    )
                )
                ...
                if T_COMP in rule:
                    ...
                    print("!" + "=" * 25 + "+" + "=" * 61 + "\\")
                    print(
                        "!{} Fragmentation mode : {:<8} header dtag{:2} Window {:2} FCN {:2} {:20}{:2} {}!".format(
                            dir_c,
                    ...
                    if T_FRAG_TILE in rule[T_FRAG][T_FRAG_PROF]:
                        txt = "Tile size: " + str(
                            rule[T_FRAG][T_FRAG_PROF][T_FRAG_TILE]
                        )
                    else:
                    ...
                    print(
                        "!{} RCS Algorithm: {:<69}{}!".format(
                            dir_c, rule[T_FRAG][T_FRAG_PROF][T_FRAG_MIC], dir_c
                        )
                    )
                    ...
                    if rule[T_FRAG][T_FRAG_MODE] != T_FRAG_NO_ACK:
                        ...
                        if rule[T_FRAG][T_FRAG_MODE] == T_FRAG_ACK_ON_ERROR:
                            txt = (
                                "Ack behavior: "
                                + rule[T_FRAG][T_FRAG_PROF][T_FRAG_ACK_BEHAVIOR]
                            )
                            print("!{} {:<84}{}!".format(dir_c, txt, dir_c))

                        print(
                            "!{} Max Retry : {:4}   Timeout {:5} seconds {:42} {}!".format(
                                dir_c,
            ...
            for rule in dev["SoR"]:
                ...
                if T_COMP in rule:
                    ...
                    frag_rule = {
                        ...
                        "rcs-algorithm": YANG_ID[rule[T_FRAG][T_FRAG_PROF][T_FRAG_MIC]][
                            1
                        ],
    ...
    def to_yang_coreconf(self, format="json"):
        ...
        for dev in self._ctxt:
            ...
            for rule in dev["SoR"]:
                ...
                if T_COMP in rule:
                    print("comp")
                    entry_sid = self.sid_search_for(
                        name="/ietf-schc:schc/rule/entry", space="data"
                    )
                    ...
                    for e in rule[T_COMP]:
                        ...
                        entry_cbor = cbor.dumps(
                            self.sid_search_for(
                                name="/ietf-schc:schc/rule/entry/field-id", space="data"
                            )
                            - entry_sid
                        ) + cbor.dumps(
                            self.sid_search_for(
                                name=YANG_ID[e[T_FID]][1], space="identity"
                            )
                        )
                        nb_elm += 1
                        ...
                        if type(l) == int:
                            entry_cbor += cbor.dumps(
                                self.sid_search_for(
                                    name="/ietf-schc:schc/rule/entry/field-length",
                                    space="data",
                                )
                                - entry_sid
                            ) + cbor.dumps(l)
                        ...
                        entry_cbor += cbor.dumps(
                            self.sid_search_for(
                                name="/ietf-schc:schc/rule/entry/field-position",
                                space="data",
                            )
                            - entry_sid
                        ...
                        entry_cbor += cbor.dumps(
                            self.sid_search_for(
                                name="/ietf-schc:schc/rule/entry/direction-indicator",
                                space="data",
                            )
                            - entry_sid
                        ) + cbor.dumps(
                            self.sid_search_for(
                                name=YANG_ID[e[T_DI]][1], space="identity"
                            )
                        )
                        nb_elm += 1
                        ...
                        entry_cbor += cbor.dumps(
                            self.sid_search_for(
                                name="/ietf-schc:schc/rule/entry/matching-operator",
                                space="data",
                            )
                            - entry_sid
                        ) + cbor.dumps(
                            self.sid_search_for(
                                name=YANG_ID[e[T_MO]][1], space="identity"
                            )
                        )
                        nb_elm += 1
                        ...
                        entry_cbor += cbor.dumps(
                            self.sid_search_for(
                                name="/ietf-schc:schc/rule/entry/comp-decomp-action",
                                space="data",
                            )
                            - entry_sid
                        ) + cbor.dumps(
                            self.sid_search_for(
                                name=YANG_ID[e[T_CDA]][1], space="identity"
                            )
                        )
                        nb_elm += 1
                        ...
                        def dictify_cbor(val, ref_id):
                            ...
                            for i in range(len(val)):
                                ...
                                tv_array += (
                                    ...
                                    + cbor.dumps(
                                        self.sid_search_for(
                                            name=ref_id + "/position", space="data"
                                        )
                                ...
                                tv_array += cbor.dumps(
                                    self.sid_search_for(
                                        name=ref_id + "/value", space="data"
                                    )
                                    - self.sid_search_for(name=ref_id, space="data")
                                ) + cbor.dumps(r)

                            tv_array = (
                                self.cbor_header(0b100_00000, len(val)) + tv_array
                            )
                            return tv_array

                        if T_TV in e and e[T_TV] != None:
                            tv_cbor = dictify_cbor(
                                e[T_TV], "/ietf-schc:schc/rule/entry/target-value"
                            )

                            entry_cbor += (
                                cbor.dumps(
                                    self.sid_search_for(
                                        name="/ietf-schc:schc/rule/entry/target-value",
                                        space="data",
                                    )
                                    - entry_sid
                                )
                        ...
                        entry_cbor = (
                            self.cbor_header(0b101_00000, nb_elm) + entry_cbor
                        )  # header MAP and size
                    ...
                    rule_content = (
                        ...
                        + self.cbor_header(0b100_00000, nb_entry)
                        + rule_content
                        + cbor.dumps(
                            self.sid_search_for(
                                name="/ietf-schc:schc/rule/rule-id-value", space="data"
                            )
                            - rule_sid
                        )
                        ...
                        + cbor.dumps(
                            self.sid_search_for(
                                name="/ietf-schc:schc/rule/rule-id-length", space="data"
                            )
                            - rule_sid
                        )
                    ...
                    rule_content += cbor.dumps(
                        self.sid_search_for(
                            name="/ietf-schc:schc/rule/direction", space="data"
                        )
                        - rule_sid
                    ) + cbor.dumps(
                        self.sid_search_for(
                            name=YANG_ID[rule[T_FRAG][T_FRAG_DIRECTION]][1],
                            space="identity",
                        )
                    )
                    nb_elm += 1
                    ...
                    rule_content += cbor.dumps(
                        self.sid_search_for(
                            name="/ietf-schc:schc/rule/rcs-algorithm", space="data"
                        )
                        - rule_sid
                    ) + cbor.dumps(
                        self.sid_search_for(
                            name=YANG_ID[rule[T_FRAG][T_FRAG_PROF][T_FRAG_MIC]][1],
                            space="identity",
                        )
                    )
                    nb_elm += 1
                    ...
                    rule_content += cbor.dumps(
                        self.sid_search_for(
                            name="/ietf-schc:schc/rule/dtag-size", space="data"
                        )
                        - rule_sid
                    ...
                    rule_content += cbor.dumps(
                        self.sid_search_for(
                            name="/ietf-schc:schc/rule/fcn-size", space="data"
                        )
                        - rule_sid
                    ...
                    rule_content += cbor.dumps(
                        self.sid_search_for(
                            name="/ietf-schc:schc/rule/fragmentation-mode", space="data"
                        )
                        - rule_sid
                    ) + cbor.dumps(
                        self.sid_search_for(
                            name=YANG_ID[rule[T_FRAG][T_FRAG_MODE]][1], space="identity"
                        )
                    )
                    nb_elm += 1
    ...
    def MO_MSB(self, TV, FV, rlength, flength, arg):
        dprint("MSB")
        dprint(f"{TV=}, {FV=}, {rlength=}, {flength=}, {arg=}")
        ...
        if type(TV) == int:
            ...
            shift = rlength - arg
            return self.MO_EQUAL(
                TV >> shift, FV >> shift, rlength - shift, flength - shift, 0
            )
    ...
    def MO_MMAP(self, TV, FV, rlength, flength, arg):
        for v in TV:
            if self.MO_EQUAL(v, FV, rlength, flength, arg):
                return True
        return False
        ...
        for d in self._ctxt:
            ...
            if d["DeviceID"] == device:  # look for a specific device
                for r in d["SoR"]:
                    ...
                    dprint(f"{tested_rule=}, {ruleID=}")
                    if tested_rule == ruleID:
                        dprint(f"rule {ruleID} found!")
                        return r
    ...
    def FindRuleFromPacket(self, pkt, direction=T_DIR_BI, failed_field=False):
        ...
        for dev in self._ctxt:
            for rule in dev["SoR"]:
                dprint(f"Checking rule {rule[T_RULEID]}/{rule[T_RULEIDLENGTH]}")
                if "Compression" in rule:
                    dprint("Compression in rule, continue")
                    matches = 0
                    for r in rule["Compression"]:
                        ...
                        if r[T_DI] == T_DIR_BI or r[T_DI] == direction:
                            if (r[T_FID], r[T_FP]) in pkt:
                                ...
                                if self.MO_function[r[T_MO]](
                                    self,
                                    r[T_TV],
                                    pkt[(r[T_FID], r[T_FP])][0],
                                    r[T_FL],
                                    pkt[(r[T_FID], r[T_FP])][1],
                                    arg,
                                ):
                                    matches += 1
                                else:
                                    if failed_field:
                                        dprint(
                                            "rule {}/{}: field {}  does not match TV={} FV={} rlen={} flen={} arg={}".format(
                                                rule[T_RULEID],
                                                rule[T_RULEIDLENGTH],
                                                r[T_FID],
                                                r[T_TV],
                                    ...
                                    dprint(f"Field/rule does not match, discontinue")
                                    break  # field does not match, rule does not match
                            else:
                                if (
                                    r[T_FL] == "var"
                                ):  # entry not found, but variable length => accept
                            ...
                            dprint(f"-> Bad direction")
                    dprint(
                        f"{'-'*10} {matches=} {len(pkt)=} {rule[T_META][T_UP_RULES]=} {rule[T_META][T_DW_RULES]=}"
                    )

                    if direction == T_DIR_UP and matches == rule[T_META][T_UP_RULES]:
                        dprint(f"Full up match, find rule from packet done!")
                        return rule, dev[T_DEVICEID]
                    if direction == T_DIR_DW and matches == rule[T_META][T_DW_RULES]:
                        dprint(f"Full dw match, find rule from packet done!")
                        return rule, dev[T_DEVICEID]

                else:
                    dprint("NO compression in rule, discontinue")
        return None, None
    ...
    def FindFragmentationRule(
        self,
        deviceID=None,
        originalSize=None,
        reliability=T_FRAG_NO_ACK,
        direction=T_DIR_UP,
        packet=None,
    ):
        ...
        for k in ["fragSender", "fragReceiver", "comp"]:
            ...
            if r is not None:
                if rule_id_length == r.RuleIDLength and rule_id == r.RuleID:
                    raise ValueError(
                        "Rule {}/{} exists".format(rule_id, rule_id_length)
                    )
    ...
    def check_rule_fragmentation(self, rule):
        ...
        if "Fragmentation" in rule:
            ...
            if not "FRMode" in fragRule:
                raise ValueError(
                    "{} Fragmentation mode must be specified".format(
                        self._nameRule(rule)
                    )
                )
            ...
            if not mode in (T_FRAG_NO_ACK, T_FRAG_ACK_ALWAYS, T_FRAG_ACK_ON_ERROR):
                raise ValueError(
                    "{} Unknown fragmentation mode".format(self._nameRule(rule))
                )
            ...
            if "windowSize" in profile:
                if (
                    ...
                    or profile["windowSize"] < 0
                ):
                    raise ValueError(
                        "{} illegal windowSize".format(self._nameRule(rule))
                    )
            else:
                profile["windowSize"] = (0x01 << profile["FCNSize"]) - 1
            ...
            if mode == T_FRAG_ACK_ON_ERROR:
                if not "ackBehavior" in profile:
                    raise ValueError(
                        "Ack on error behavior must be specified (afterAll1 or afterAll0)"
                    )
                if not "tileSize" in profile:
                    profile["tileSize"] = 64

    def get_init_info(self):
        result = {"context": self._ctxt.copy(), "database": self._db.copy()}
        return result
